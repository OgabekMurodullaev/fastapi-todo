...
@router.post("/", response_model=Todo, status_code=status.HTTP_201_CREATED, summary="Create a job")
def create_todo(todo: TodoCreate, db: Session = Depends(get_db)):
    todo = TodoDB.create(db, **todo.model_dump())
    return todo


@router.get("/{todo_id}", response_model=Todo, summary="Get a job")
def return_todo(todo_id: int, db: Session = Depends(get_db)):
